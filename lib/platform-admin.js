...
function promotionCustomerScope(value, legacyNewCustomerOnly = false) {
  const scope = String(value || "").trim().toLowerCase();
  if (["new", "existing", "both"].includes(scope)) return scope;
  return legacyNewCustomerOnly ? "new" : "both";
}
...
function normalizePromoInput(input = {}, existingId = "") {
  ...
  const applicableBillingInput = input.applicableBilling ?? input.applicable_billing;
  const applicableBilling = Array.isArray(applicableBillingInput)
    ? [...new Set(applicableBillingInput.map(value => String(value).trim().toLowerCase()).filter(Boolean))]
    : ["monthly", "yearly"];
  ...
  if (!applicableBilling.length || applicableBilling.some(billing => !["monthly", "yearly"].includes(billing))) throw new Error("ช่วงรอบบิลที่เลือกไม่ถูกต้อง");
  ...
  const customerScope = promotionCustomerScope(input.customerScope ?? input.customer_scope, input.newCustomersOnly === true || String(input.newCustomersOnly || "").toLowerCase() === "true");
  return {
    ...
    new_customer_only: customerScope === "new",
    applicable_plans: plans,
    applicable_billing: applicableBilling,
    source_campaign: String(input.sourceCampaign ?? input.source_campaign ?? "").trim().slice(0, 200) || null
  };
}
...
function promoForUi(row = {}) {
  return {
    ...
    customerScope: promotionCustomerScope(row.customer_scope, row.new_customer_only === true),
    newCustomersOnly: row.new_customer_only === true,
    plans: Array.isArray(row.applicable_plans) ? row.applicable_plans : [],
    applicableBilling: Array.isArray(row.applicable_billing) ? row.applicable_billing : ["monthly", "yearly"],
    sourceCampaign: row.source_campaign ? String(row.source_campaign) : "",
