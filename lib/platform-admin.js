...
const PROMO_STORE_FILE = process.env.PLATFORM_ADMIN_PROMO_STORE_PATH || path.join(ROOT, "data", "platform-admin-promos.json");
...
function buildSnapshot(source, range) {
  ...
  const promoStore = readPromoStore();
  const activePromos = promoStore.promos.filter(item => item.status === "active");
  ...
  const snapshot = {
    ...
    home: {
      ...
      promos: promoStore.promos.length ? { active: activePromos.length, used: promoStore.promos.reduce((sum, item) => sum + Number(item.usedCount || 0), 0), remaining: null } : { active: null, used: null, remaining: null },
...
function readPromoStore() {
  try {
    const value = JSON.parse(fs.readFileSync(PROMO_STORE_FILE, "utf8"));
    return { promos: Array.isArray(value.promos) ? value.promos : [], auditLog: Array.isArray(value.auditLog) ? value.auditLog : [] };
  } catch {
    return { promos: [], auditLog: [] };
  }
}

function writePromoStore(store) {
  fs.mkdirSync(path.dirname(PROMO_STORE_FILE), { recursive: true });
  const temp = `${PROMO_STORE_FILE}.${process.pid}.tmp`;
  fs.writeFileSync(temp, `${JSON.stringify(store, null, 2)}\n`, { encoding: "utf8", flag: "w" });
  fs.renameSync(temp, PROMO_STORE_FILE);
}

function normalizedPromo(input = {}, existing = {}) {
  const code = String(input.code ?? existing.code ?? "").trim().toUpperCase();
  const type = ["percentage", "fixed_thb", "free_days", "free_months"].includes(String(input.type ?? existing.type)) ? String(input.type ?? existing.type) : "percentage";
  const usageLimit = String(input.usageLimit ?? existing.usageLimit ?? "").trim().toLowerCase() === "unlimited" || input.usageLimit === null ? null : Math.max(0, Number(input.usageLimit ?? existing.usageLimit ?? 0) || 0);
  return {
    ...existing,
    id: String(existing.id || `promo_${crypto.randomBytes(6).toString("hex")}`),
    code,
    description: String(input.description ?? existing.description ?? "").trim(),
    type,
    value: Math.max(0, Number(input.value ?? existing.value ?? 0) || 0),
    plans: Array.isArray(input.plans) ? input.plans.map(String).filter(Boolean).slice(0, 3) : (existing.plans || []),
    startsAt: String(input.startsAt ?? existing.startsAt ?? "").trim(),
    expiresAt: String(input.expiresAt ?? existing.expiresAt ?? "").trim(),
    usageLimit,
    usagePerCompany: Math.max(0, Number(input.usagePerCompany ?? existing.usagePerCompany ?? 1) || 1),
    newCustomersOnly: Boolean(input.newCustomersOnly ?? existing.newCustomersOnly),
    status: input.status === "disabled" || existing.status === "disabled" ? "disabled" : "active",
    usedCount: Math.max(0, Number(existing.usedCount || 0) || 0),
