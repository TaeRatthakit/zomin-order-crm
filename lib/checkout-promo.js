...
// Temporary rollout boundary, NOT an authentication bypass. Never connects
// Preview to Production and never enables this consumer on Production.
function checkoutPromoEnabled(env = process.env) {
  try {
    return env.VERCEL_ENV === "preview"
      && env.CHECKOUT_PROMO_ENABLED === "true"
      && env.DATABASE_PROVIDER === "supabase"
      && new URL(env.SUPABASE_URL).hostname === "enwabsfsmwwcwwirdwok.supabase.co";
  } catch { return false; }
}
