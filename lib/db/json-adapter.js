const fs = require("fs");
const path = require("path");
const crypto = require("crypto");
const { synchronizeCustomers } = require("../customer-sync");
const tenantPrimitives = require("./tenant-primitives");

const ROOT = path.join(__dirname, "..", "..");
const DATA_FILE = process.env.JSON_DB_PATH || path.join(ROOT, "data", "db.json");
const OPPORTUNITY_CYCLE_NOTE_RE = /\n?\[\[opportunityCycle:orderId=([^\]]+)\]\]/g;
const PRODUCT_RESOLUTION_ERROR = "ไม่พบสินค้า กรุณาเพิ่มสินค้าหรือจับคู่สินค้าให้ถูกต้องก่อนสร้างออเดอร์";

function opportunityLogOrderId(log = {}) {
  const direct = String(log.orderId || log.order_id || "").trim();
  if (direct) return direct;
  OPPORTUNITY_CYCLE_NOTE_RE.lastIndex = 0;
  const match = OPPORTUNITY_CYCLE_NOTE_RE.exec(String(log.note || ""));
  OPPORTUNITY_CYCLE_NOTE_RE.lastIndex = 0;
  return match ? decodeURIComponent(match[1]) : "";
}

function opportunityCycleMarker(orderId = "") {
  return `[[opportunityCycle:orderId=${encodeURIComponent(String(orderId || "").trim())}]]`;
}

function stripOpportunityCycleNote(note = "") {
  return String(note || "").replace(OPPORTUNITY_CYCLE_NOTE_RE, "").trim();
}

function noteWithOpportunityCycle(note = "", orderId = "") {
  return [stripOpportunityCycleNote(note), opportunityCycleMarker(orderId)].filter(Boolean).join("\n");
}

function uid(prefix) {
  return `${prefix}_${crypto.randomBytes(6).toString("hex")}`;
}

function productNameKey(value = "") {
  return String(value || "").trim().toLowerCase().replace(/\s+/g, " ");
}

function resolveImportProduct(settings = {}, row = {}) {
  const products = (Array.isArray(settings.products) ? settings.products : [])
    .filter(product => product && product.archived !== true && String(product.name || "").trim());
  const productId = String(row.productId || row.product_id || "").trim();
  const nameKey = productNameKey(row.items || row.product || row.productName || "");
  const product = (productId ? products.find(item => String(item.id || "") === productId) : null)
    || products.find(item => productNameKey(item.name) === nameKey);
  if (!product) throw new Error(PRODUCT_RESOLUTION_ERROR);
  return product;
}

function moneyMatches(left, right) {
  return Math.abs(Number(left || 0) - Number(right || 0)) < 0.000001;
}

function normalizePackageExpenses(expenses = []) {
  return (Array.isArray(expenses) ? expenses : []).map((expense, index) => ({
    id: String(expense?.id || `expense_${index + 1}`),
    name: String(expense?.name || "").trim(),
    amount: Math.max(0, Number(expense?.amount || 0)),
    enabled: expense?.enabled !== false
  })).filter(expense => expense.name || expense.amount > 0);
}

function normalizeSalesPackages(packages = []) {
  return (Array.isArray(packages) ? packages : []).map((item, index) => {
    const paidQuantity = Math.max(0, Number(item?.paidQuantity || 0));
    const freeQuantity = Math.max(0, Number(item?.freeQuantity || 0));
    return {
      id: String(item?.id || `package_${index + 1}`),
      name: String(item?.name || "").trim(),
      paidQuantity,
      freeQuantity,
      totalQuantityShipped: Math.max(0, Number(item?.totalQuantityShipped || paidQuantity + freeQuantity)),
      salePrice: Math.max(0, Number(item?.salePrice || 0)),
      enabled: item?.enabled !== false,
      expenses: normalizePackageExpenses(item?.expenses)
    };
  }).filter(item => item.id && item.totalQuantityShipped > 0);
}

function importSalesPackageForRow(product = {}, row = {}) {
  const packageId = String(row.packageId || row.package_id || "").trim();
  const packages = normalizeSalesPackages(product.salesPackages);
  if (packageId) return packages.find(item => item.id === packageId && item.enabled !== false) || null;
  const revenue = Number(row.amount || 0);
  const shipped = Number(row.totalQuantityShipped || row.jars || 0);
  if (!revenue || !shipped) return null;
  const matches = packages.filter(item =>
    item.enabled !== false &&
    moneyMatches(item.salePrice, revenue) &&
    moneyMatches(item.totalQuantityShipped, shipped)
  );
  return matches.length === 1 ? matches[0] : null;
}

function settingsCostRows(rows = [], amountField = "amount") {
  return (Array.isArray(rows) ? rows : []).map((item, index) => ({
    id: String(item?.id || `cost_${index + 1}`),
    name: String(item?.name || "").trim(),
    [amountField]: Math.max(0, Number(item?.[amountField] || 0)),
    type: String(item?.type || "fixed").trim() || "fixed",
    enabled: item?.enabled !== false
  }));
}

function snapshotMoney(value) {
  return Math.round(Number(value || 0) * 100) / 100;
}

function importProfitSnapshot(order = {}, settings = {}) {
  const revenueSnapshot = snapshotMoney(order.amount);
  const productConfig = settingsCostRows(settings.productCosts, "costPerJar")
    .find(item => item.id === order.productId || productNameKey(item.name) === productNameKey(order.items));
  const productQuantity = Number(order.totalQuantityShipped || order.jars || 0);
  const productCostSnapshot = productConfig?.enabled
    ? snapshotMoney(productQuantity * Number(productConfig.costPerJar || 0))
    : 0;
  const packageExpenseSnapshot = snapshotMoney(normalizePackageExpenses(order.packageExpenses)
    .filter(expense => expense.enabled)
    .reduce((sum, expense) => sum + Number(expense.amount || 0), 0));
  const globalExpenseSnapshot = snapshotMoney(settingsCostRows(settings.additionalCosts, "amount")
    .filter(expense => expense.enabled)
    .reduce((sum, expense) => {
      if (expense.type === "percent_sales") return sum + (revenueSnapshot * Number(expense.amount || 0) / 100);
      if (expense.type === "per_item") return sum + (Number(order.jars || 0) * Number(expense.amount || 0));
      return sum + Number(expense.amount || 0);
    }, 0));
  const profitBeforeAdsSnapshot = snapshotMoney(
    revenueSnapshot - productCostSnapshot - packageExpenseSnapshot - globalExpenseSnapshot
  );
  const timestamp = new Date().toISOString();
  return {
    revenueSnapshot,
    productCostSnapshot,
    packageExpenseSnapshot,
    globalExpenseSnapshot,
    profitBeforeAdsSnapshot,
    profitAfterAdsSnapshot: profitBeforeAdsSnapshot,
    profitSnapshotVersion: 2,
    profitSnapshotCreatedAt: timestamp,
    profitSnapshotUpdatedAt: timestamp,
    profitSnapshotSource: "import"
  };
}

function importPackageFields(settings = {}, product = {}, row = {}) {
  const selectedPackage = importSalesPackageForRow(product, row);
  const fields = {
    productId: product.id || "",
    packageId: selectedPackage?.id || "",
    packageName: selectedPackage?.name || "",
    paidQuantity: selectedPackage ? Number(selectedPackage.paidQuantity || 0) : Number(row.paidQuantity || 0),
    freeQuantity: selectedPackage ? Number(selectedPackage.freeQuantity || 0) : Number(row.freeQuantity || 0),
    totalQuantityShipped: selectedPackage ? Number(selectedPackage.totalQuantityShipped || 0) : Number(row.totalQuantityShipped || row.jars || 0),
    packageExpenses: selectedPackage ? normalizePackageExpenses(selectedPackage.expenses) : normalizePackageExpenses(row.packageExpenses)
  };
  return {
    ...fields,
    snapshotFor(order) {
      return importProfitSnapshot({ ...order, ...fields }, settings);
    }
  };
}

function readDb() {
  const db = JSON.parse(fs.readFileSync(DATA_FILE, "utf8"));
  const originalCustomers = Array.isArray(db.customers)
    ? db.customers.map(customer => ({ ...customer, tags: Array.isArray(customer.tags) ? [...customer.tags] : customer.tags }))
    : [];
  const synced = synchronizeCustomers(db);
  synced.subscriptions = Array.isArray(synced.subscriptions) ? synced.subscriptions : [];
  synced.payments = Array.isArray(synced.payments) ? synced.payments : [];
  const liveCustomerIds = new Set((synced.customers || []).map(customer => String(customer.id || "")));
  synced.customers = [
    ...(synced.customers || []),
    ...originalCustomers.filter(customer => !liveCustomerIds.has(String(customer.id || "")))
  ];
  return synced;
}

async function unsupportedSupabaseFeature(feature) {
  const error = new Error(`${feature} requires the Supabase adapter.`);
  error.code = "SUPABASE_FEATURE_UNSUPPORTED";
  throw error;
}

function findUserForLogin(username) {
  const value = String(username || "").trim();
  if (!value) return null;
  const db = readDb();
  return (db.users || []).find(user =>
    user.active !== false &&
    (user.username === value || user.id === value)
  ) || null;
}

function readUserById(id) {
  const value = String(id || "").trim();
  if (!value) return null;
  const db = readDb();
  return (db.users || []).find(user => user.id === value) || null;
}

function writeDb(db, options = {}) {
  const persisted = options.allowCustomerDeletion
    ? null
    : JSON.parse(fs.readFileSync(DATA_FILE, "utf8"));
  const originalCustomers = [
    ...(Array.isArray(db.customers) ? db.customers : []),
    ...(!persisted || !Array.isArray(persisted.customers) ? [] : persisted.customers)
  ].reduce((customers, customer) => {
    if (!customers.some(existing => String(existing.id || "") === String(customer.id || ""))) {
      customers.push({ ...customer, tags: Array.isArray(customer.tags) ? [...customer.tags] : customer.tags });
    }
    return customers;
  }, []);
  const synced = synchronizeCustomers(db);
  const liveCustomerIds = new Set((synced.customers || []).map(customer => String(customer.id || "")));
  const preservedCustomers = originalCustomers.filter(customer => !liveCustomerIds.has(String(customer.id || "")));
  synced.customers = [...(synced.customers || []), ...preservedCustomers];
  fs.writeFileSync(DATA_FILE, `${JSON.stringify(synced, null, 2)}\n`, "utf8");
}

function persistSettingsPatch(patch = {}) {
  const db = readDb();
  db.settings = {
    ...(db.settings || {}),
    ...patch
  };
  writeDb(db);
}

function readSettingsPatch(keys = []) {
  const db = readDb();
  const names = [...new Set(keys.map(key => String(key || "").trim()).filter(Boolean))];
  return Object.fromEntries(names.map(key => [key, db.settings?.[key]]));
}

function sortedOpportunityOrders(orders = []) {
  return [...orders].sort((a, b) => [
    String(a.date || ""),
    String(a.time || ""),
    String(a.id || "")
  ].join("|").localeCompare([
    String(b.date || ""),
    String(b.time || ""),
    String(b.id || "")
  ].join("|")));
}

function inferOpportunityLogOrderId(orders = [], log = {}) {
  const direct = opportunityLogOrderId(log);
  if (direct) return direct;
  const sorted = sortedOpportunityOrders(orders);
  if (sorted.length === 1) return String(sorted[0].id || "");
  const logDate = String(log.date || log.contact_date || "");
  const previousOrders = sorted.filter(order => String(order.date || "") <= logDate);
  return previousOrders.length ? String(previousOrders[previousOrders.length - 1].id || "") : "";
}

function createContactLogFast(input = {}) {
  const timings = {};
  const readStartedAt = Date.now();
  const db = readDb();
  const customerId = String(input.customerId || "").trim();
  const customer = (db.customers || []).find(item => item.id === customerId);
  const orders = sortedOpportunityOrders((db.orders || []).filter(order => order.customerId === customerId));
  const duplicateRows = (db.contactLogs || []).filter(log => log.customerId === customerId && log.result === input.result);
  timings.readMs = Date.now() - readStartedAt;
  if (!customer) return { ok: false, status: 404, error: "ไม่พบลูกค้า", timings };
  const manualOpportunityResult = Boolean(input.manualOpportunityResult);
  const requestedOrderId = String(input.orderId || "").trim();
  const requestedOrder = requestedOrderId ? orders.find(order => order.id === requestedOrderId) : null;
  if (requestedOrderId && !requestedOrder) {
    return { ok: false, status: 400, error: "รอบออเดอร์นี้ไม่ตรงกับลูกค้า", timings };
  }
  const cycleOrder = requestedOrder || (manualOpportunityResult ? orders[orders.length - 1] : null);
  const cycleOrderId = String(cycleOrder?.id || "");
  const duplicateStartedAt = Date.now();
  if (manualOpportunityResult) {
    const existing = duplicateRows.find(log => {
      if (log.customerId !== customerId || log.result !== input.result) return false;
      if (!cycleOrderId) return log.date === input.date;
      return inferOpportunityLogOrderId(orders, log) === cycleOrderId;
    });
    timings.duplicateMs = Date.now() - duplicateStartedAt;
    if (existing) return { ok: true, log: { ...existing, orderId: inferOpportunityLogOrderId(orders, existing) }, duplicate: true, timings };
  } else {
    timings.duplicateMs = Date.now() - duplicateStartedAt;
  }
  const log = {
    id: uid("log"),
    customerId,
    date: input.date,
    result: String(input.result || "โทรติด").trim() || "โทรติด",
    note: manualOpportunityResult && cycleOrderId
      ? noteWithOpportunityCycle(input.note || "", cycleOrderId)
      : String(input.note || "").trim(),
    staff: String(input.staff || "").trim(),
    nextFollowUpDate: input.nextFollowUpDate || "",
    createdAt: new Date().toISOString(),
    orderId: cycleOrderId
  };
  const writeStartedAt = Date.now();
  customer.lastContactDate = log.date;
  customer.lastContactNote = log.note;
  db.contactLogs = [log, ...(db.contactLogs || [])];
  writeDb(db);
  timings.writeMs = Date.now() - writeStartedAt;
  timings.serializeMs = 0;
  return { ok: true, log, duplicate: false, timings };
}

function readNotificationReadIds(userId) {
  const db = readDb();
  return (db.notificationReads || [])
    .filter(item => item.userId === userId)
    .map(item => item.notificationId);
}

function persistNotificationReadIds(userId, notificationIds = []) {
  const db = readDb();
  const ids = [...new Set((notificationIds || []).map(id => String(id || "").trim()).filter(Boolean))];
  db.notificationReads = Array.isArray(db.notificationReads) ? db.notificationReads : [];
  const known = new Set(db.notificationReads
    .filter(item => item.userId === userId)
    .map(item => item.notificationId));
  const readAt = new Date().toISOString();
  for (const notificationId of ids) {
    if (known.has(notificationId)) continue;
    db.notificationReads.push({ userId, notificationId, readAt });
  }
  writeDb(db);
  return readNotificationReadIds(userId);
}

function persistOrderProfitSnapshots(orders = []) {
  if (!orders.length) return;
  const db = readDb();
  const snapshotFields = [
    "revenueSnapshot",
    "productCostSnapshot",
    "packageExpenseSnapshot",
    "globalExpenseSnapshot",
    "profitBeforeAdsSnapshot",
    "profitAfterAdsSnapshot",
    "profitSnapshotVersion",
    "profitSnapshotCreatedAt",
    "profitSnapshotUpdatedAt",
    "profitSnapshotSource"
  ];
  const snapshotsById = new Map(orders.map(order => [order.id, order]));
  for (const storedOrder of db.orders || []) {
    const snapshot = snapshotsById.get(storedOrder.id);
    if (!snapshot) continue;
    for (const field of snapshotFields) storedOrder[field] = snapshot[field];
  }
  writeDb(db);
}

function persistUserProfile(userId, { displayName, avatar }) {
  const db = readDb();
  const user = (db.users || []).find(item => item.id === userId);
  if (!user) return null;
  user.name = displayName;
  user.avatar = avatar;
  writeDb(db);
  return user;
}

function persistUserThemePreference(userId, themePreference) {
  const db = readDb();
  const user = (db.users || []).find(item => item.id === userId);
  if (!user) return null;
  user.themePreference = ["dark", "light", "system"].includes(themePreference) ? themePreference : "system";
  writeDb(db);
  return user;
}

function deleteOrder(id) {
  const db = readDb();
  const orderIndex = db.orders.findIndex(order => order.id === id);
  if (orderIndex === -1) return false;
  db.orders.splice(orderIndex, 1);
  writeDb(db);
  return true;
}

function deleteCustomer(id, audit = {}) {
  const db = readDb();
  const customerIndex = db.customers.findIndex(customer => customer.id === id);
  if (customerIndex === -1) return false;
  if ((db.orders || []).some(order => order.customerId === id)) return false;
  const actorUserId = String(audit.actorUserId || audit.actor_user_id || "").trim();
  const actorRole = String(audit.actorRole || audit.actor_role || "").trim();
  const tenantId = String(audit.tenantId || audit.tenant_id || "").trim();
  if (!actorUserId || !actorRole || !tenantId) {
    const error = new Error("Customer deletion requires authenticated tenant actor context.");
    error.code = "CUSTOMER_DELETE_AUDIT_CONTEXT_REQUIRED";
    throw error;
  }
  db.customerDeletionAudits = Array.isArray(db.customerDeletionAudits) ? db.customerDeletionAudits : [];
  const deletedAt = new Date().toISOString();
  db.customerDeletionAudits.push({
    id: "customer_delete_audit_" + Date.now() + "_" + Math.random().toString(16).slice(2, 10),
    customerId: id,
    tenantId,
    actorUserId,
    actorRole,
    action: "customer_delete",
    deletedAt,
    customerSnapshot: JSON.parse(JSON.stringify(db.customers[customerIndex])),
    requestMetadata: audit.requestMetadata && typeof audit.requestMetadata === "object"
      ? {
        route: String(audit.requestMetadata.route || "").slice(0, 200),
        method: String(audit.requestMetadata.method || "").slice(0, 16),
        source: String(audit.requestMetadata.source || "").slice(0, 80),
        request_id: String(audit.requestMetadata.request_id || "").slice(0, 160),
        user_agent: String(audit.requestMetadata.user_agent || "").slice(0, 240)
      }
      : {},
    createdAt: deletedAt
  });
  db.customers.splice(customerIndex, 1);
  db.contactLogs = (db.contactLogs || []).filter(log => log.customerId !== id);
  writeDb(db, { allowCustomerDeletion: true });
  return true;
}

function deleteUser(id) {
  const db = readDb();
  const before = (db.users || []).length;
  db.users = (db.users || []).filter(user => user.id !== id);
  if (db.users.length === before) return false;
  writeDb(db);
  return true;
}

function getImportJob(id) {
  const db = readDb();
  return db.importJobs?.find(job => job.id === id) || null;
}

function getActiveImportJob(type) {
  const db = readDb();
  return db.importJobs?.find(job => job.type === type && ["queued", "running", "paused"].includes(job.status)) || null;
}

function getLatestImportJob(type) {
  const db = readDb();
  return (db.importJobs || [])
    .filter(job => job.type === type)
    .sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")))[0] || null;
}

function saveImportJob(job) {
  const db = readDb();
  db.importJobs = db.importJobs || [];
  const index = db.importJobs.findIndex(item => item.id === job.id);
  if (index === -1) db.importJobs.push(job);
  else db.importJobs[index] = job;
  writeDb(db);
  return job;
}

function importOrdersBatch(rows, options = {}) {
  const db = readDb();
  const imported = [];
  const importedCustomerIds = [];
  const failed = [];
  let skipped = 0;
  const existingKeys = new Set((db.orders || []).map(order => {
    const orderNumber = String(order.orderNumber || "").trim().toLowerCase();
    return orderNumber
      ? `order:${orderNumber}`
      : `fallback:${order.date}|${String(order.phone || "").replace(/[^\d]/g, "")}|${Number(order.amount || 0)}`;
  }));

  for (const row of rows) {
    try {
      const phone = String(row.phone || "").replace(/[^\d]/g, "");
      if (!phone || !row.name || !row.date) throw new Error("ข้อมูลชื่อ เบอร์โทร หรือวันที่ไม่ครบ");
      const orderNumber = String(row.orderNumber || "").trim().toLowerCase();
      const key = orderNumber
        ? `order:${orderNumber}`
        : `fallback:${row.date}|${phone}|${Number(row.amount || 0)}`;
      if (existingKeys.has(key)) {
        skipped += 1;
        continue;
      }
      const product = resolveImportProduct(db.settings || {}, row);
      const packageFields = importPackageFields(db.settings || {}, product, row);

      let customer = db.customers.find(item => String(item.phone || "").replace(/[^\d]/g, "") === phone);
      if (!customer) {
        customer = {
          id: `c_${require("crypto").randomBytes(6).toString("hex")}`,
          name: String(row.name).trim(),
          phone,
          address: String(row.address || "").trim(),
          tags: Array.isArray(row.tags) ? row.tags : String(row.tags || "").split(",").map(tag => tag.trim()).filter(Boolean),
          note: "",
          createdAt: row.date,
          lastContactDate: "",
          lastContactNote: "",
          assignedTo: ""
        };
        db.customers.push(customer);
        importedCustomerIds.push(customer.id);
      }
      const order = {
        importJobId: String(options.jobId || ""),
        id: `o_${require("crypto").randomBytes(6).toString("hex")}`,
        customerId: customer.id,
        orderNumber: String(row.orderNumber || "").trim(),
        customerName: String(row.name).trim(),
        phone,
        address: String(row.address || "").trim(),
        date: row.date,
        time: row.time || "",
        items: product.name,
        jars: Number(row.jars || 1),
        amount: Number(row.amount || 0),
        source: "Import",
        sourceChannel: row.sourceChannel || "Import",
        socialName: row.socialName || "",
        alternatePhone: row.alternatePhone || "",
        originSource: row.originSource || "",
        originSourceOther: row.originSourceOther || "",
        freeGift: row.freeGift || "",
        vipCardStatus: row.vipCardStatus || "",
        note: row.note || "",
        rawText: row.rawText || "",
        ...packageFields
      };
      delete order.snapshotFor;
      Object.assign(order, packageFields.snapshotFor(order));
      db.orders.push(order);
      imported.push(order.id);
      existingKeys.add(key);
    } catch (error) {
      failed.push({ rowNumber: row.rowNumber, error: error.message, row });
    }
  }
  writeDb(db);
  return { imported: imported.length, skipped, failed, importedOrderIds: imported, importedCustomerIds };
}

function previewLatestImportCleanup(type = "orders") {
  const db = readDb();
  const job = getLatestImportJob(type);
  if (!job) return null;
  const orderIds = Array.isArray(job.importedOrderIds) ? job.importedOrderIds : [];
  const customerIds = Array.isArray(job.importedCustomerIds) ? job.importedCustomerIds : [];
  return {
    job,
    orderCount: orderIds.length,
    orderIds,
    customerCount: customerIds.length,
    customerIds,
    settingsKeys: [`import_job_${job.id}`],
    supported: true
  };
}

function cleanupImportJob(jobId, audit = {}) {
  const db = readDb();
  const job = db.importJobs?.find(item => item.id === jobId);
  if (!job) return null;
  const orderIds = Array.isArray(job.importedOrderIds) ? job.importedOrderIds : [];
  const orderIdSet = new Set(orderIds);
  const listedOrders = (db.orders || []).filter(order => orderIdSet.has(order.id));
  if (listedOrders.length !== orderIds.length || listedOrders.some(order => String(order.importJobId || "") !== String(jobId))) {
    const error = new Error("Import cleanup provenance could not be proven.");
    error.code = "IMPORT_CLEANUP_ORDER_PROVENANCE_MISMATCH";
    throw error;
  }
  const actorUserId = String(audit.actorUserId || audit.actor_user_id || "").trim();
  const actorRole = String(audit.actorRole || audit.actor_role || "").trim();
  const tenantId = String(audit.tenantId || audit.tenant_id || "").trim();
  if (!actorUserId || !actorRole || !tenantId) {
    const error = new Error("Import cleanup requires authenticated tenant actor context.");
    error.code = "IMPORT_CLEANUP_AUDIT_CONTEXT_REQUIRED";
    throw error;
  }
  db.orderDeletionAudits = Array.isArray(db.orderDeletionAudits) ? db.orderDeletionAudits : [];
  const requestMetadata = audit.requestMetadata && typeof audit.requestMetadata === "object"
    ? {
      route: String(audit.requestMetadata.route || "").slice(0, 200),
      method: String(audit.requestMetadata.method || "").slice(0, 16),
      source: "import_cleanup",
      request_id: String(audit.requestMetadata.request_id || "").slice(0, 160),
      user_agent: String(audit.requestMetadata.user_agent || "").slice(0, 240)
    }
    : { source: "import_cleanup" };
  const deletedAt = new Date().toISOString();
  for (const order of listedOrders) {
    db.orderDeletionAudits.push({
      id: "order_delete_audit_" + Date.now() + "_" + Math.random().toString(16).slice(2, 10),
      orderId: order.id,
      tenantId,
      actorUserId,
      actorRole,
      action: "order_delete",
      deletedAt,
      orderSnapshot: JSON.parse(JSON.stringify(order)),
      requestMetadata,
      createdAt: deletedAt
    });
  }
  if (orderIds.length) {
    db.orders = (db.orders || []).filter(order => !orderIds.includes(order.id));
  }
  db.importJobs = (db.importJobs || []).filter(item => item.id !== jobId);
  writeDb(db);
  return {
    job,
    deletedOrders: orderIds.length,
    deletedCustomers: 0,
    deletedImportRecords: 1,
    supported: true
  };
}

function verifyCustomerSync() {
  const raw = JSON.parse(fs.readFileSync(DATA_FILE, "utf8"));
  const projected = synchronizeCustomers(JSON.parse(JSON.stringify(raw)));
  const rawCustomerIds = new Set((raw.customers || []).map(customer => customer.id));
  const projectedCustomerIds = new Set((projected.customers || []).map(customer => customer.id));
  const orderCustomerIds = new Set((raw.orders || []).map(order => order.customerId).filter(Boolean));
  const orphanCustomerCount = [...rawCustomerIds].filter(id => !orderCustomerIds.has(id)).length;
  const missingCustomerCount = [...orderCustomerIds].filter(id => !rawCustomerIds.has(id)).length;
  const projectedOnlyCustomerCount = [...projectedCustomerIds].filter(id => !rawCustomerIds.has(id)).length;
  const staleCustomerCount = [...rawCustomerIds].filter(id => !projectedCustomerIds.has(id)).length;
  return {
    ok: orphanCustomerCount === 0 && missingCustomerCount === 0 && projectedOnlyCustomerCount === 0 && staleCustomerCount === 0,
    provider: "json",
    orderCount: (raw.orders || []).length,
    storedCustomerCount: (raw.customers || []).length,
    projectedCustomerCount: (projected.customers || []).length,
    orphanCustomerCount,
    missingCustomerCount,
    projectedOnlyCustomerCount,
    staleCustomerCount
  };
}

module.exports = {
  provider: "json",
  readDb,
  findUserForLogin,
  readUserById,
  createSignupTenantAccount: async () => {
    const error = new Error("Signup tenant bootstrap requires the Supabase production adapter.");
    error.code = "SIGNUP_PROVIDER_UNSUPPORTED";
    throw error;
  },
  validatePromotionCode: async () => {
    const error = new Error("Promotion code validation requires the Supabase adapter.");
    error.code = "SIGNUP_PROVIDER_UNSUPPORTED";
    throw error;
  },
  beginSubscriptionPayment: async () => unsupportedSupabaseFeature("Subscription payment"),
  beginSubscriptionCheckout: async () => unsupportedSupabaseFeature("Subscription checkout"),
  beginSubscriptionUpgrade: async () => unsupportedSupabaseFeature("Subscription upgrade"),
  setPaymentProviderReference: async () => unsupportedSupabaseFeature("Subscription payment"),
  recordProviderPaymentSuccess: async () => unsupportedSupabaseFeature("Subscription payment"),
  recordSubscriptionCheckoutSuccess: async () => unsupportedSupabaseFeature("Subscription checkout"),
  recordProviderPaymentStatus: async () => unsupportedSupabaseFeature("Subscription payment"),
  recordSubscriptionUpgradeSuccess: async () => unsupportedSupabaseFeature("Subscription upgrade"),
  recordSubscriptionUpgradeStatus: async () => unsupportedSupabaseFeature("Subscription upgrade"),
  activateZeroAmountSubscriptionPayment: async () => unsupportedSupabaseFeature("Subscription payment"),
  readPaymentByProviderReference: async () => null,
  readPendingSubscriptionUpgrades: async () => [],
  closeTerminalSubscriptionUpgradeAttempt: async () => null,
  platformAdminOverview: async () => unsupportedSupabaseFeature("Platform admin"),
  platformAdminTenants: async () => unsupportedSupabaseFeature("Platform admin"),
  platformAdminTenantDetail: async () => unsupportedSupabaseFeature("Platform admin"),
  platformAdminPayments: async () => unsupportedSupabaseFeature("Platform admin"),
  platformAdminPromotionCodes: async () => unsupportedSupabaseFeature("Platform admin"),
  platformAdminUpsertPromotionCode: async () => unsupportedSupabaseFeature("Platform admin"),
  writeDb,
  deleteUser,
  deleteOrder,
  deleteCustomer,
  getImportJob,
  getActiveImportJob,
  getLatestImportJob,
  previewLatestImportCleanup,
  cleanupImportJob,
  saveImportJob,
  importOrdersBatch,
  persistOrderProfitSnapshots,
  createContactLogFast,
  persistUserProfile,
  persistUserThemePreference,
  persistSettingsPatch,
  readSettingsPatch,
  readNotificationReadIds,
  persistNotificationReadIds,
  withTenantContext: (context, task) => task(),
  resolveTenantForUser: async () => null,
  resolveTenantForLineWebhook: async () => null,
  verifyCustomerSync,
  tenantPrimitives,
  DATA_FILE
};
