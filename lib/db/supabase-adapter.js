...
function tenantOwnedRow(table, row = {}) {
  ...
  if (table === "settings" && next.key) next.id = `${context.tenantId}:${next.key}`;
  if (table === "follow_up_rules" && next.jars !== undefined) next.id = `${context.tenantId}:${next.jars}`;
  if (table === "tags" && next.name) next.id = `${context.tenantId}:${next.name}`;
...
async function resolveTenantForLineWebhook(body = {}) {
  ...
  if (configuredTenantId && configuredGroupId && groupIds.length === 1 && groupIds[0] === configuredGroupId) {
...
async function createSignupTenantAccount(input = {}) {
  const rows = await rpc("growup_signup_bootstrap", {
    ...
    for (const code of ["PROMOTION_CODE_INVALID", "PROMOTION_CODE_EXPIRED", "PROMOTION_CODE_EXHAUSTED"]) {
