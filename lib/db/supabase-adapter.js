...
async function resolveTenantForLineWebhook(body = {}) {
  ...
  for (const row of settingsRows || []) {
    ...
    if (row.key === "lineGroupIds") {
      ...
      const trustedGroups = new Set(row.value.map(item => String(item || "").trim()));
      if (trustedGroups.has(groupId)) candidateTenantIds.add(tenantId);
      continue;
    }
    if (row.key === "lineGroupId" && String(row.value || "").trim() === groupId) {
      candidateTenantIds.add(tenantId);
    }
  }
  for (const tenantId of malformedTenantIds) {
    candidateTenantIds.delete(tenantId);
  }
...
async function diagnoseLineWebhookTenantRejection(body = {}) {
  ...
  for (const row of settingsRows || []) {
    ...
    if (row.key === "lineGroupIds") {
      ...
      if (new Set(row.value.map(item => String(item || "").trim())).has(groupId)) {
        candidateTenantIds.add(tenantId);
      }
      continue;
    }
