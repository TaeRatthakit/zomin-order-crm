...
async function persistOrderDeletion(id, audit = {}) {
  const context = requireTenantContext("orders", "delete");
  const actorUserId = String(audit.actorUserId || audit.actor_user_id || context?.userId || "").trim();
  const actorRole = String(audit.actorRole || audit.actor_role || context?.tenantRole || "").trim();
  if (!context?.tenantId || !actorUserId || actorUserId !== String(context.userId || "").trim() || !actorRole) {
    const error = new Error("Order deletion requires authenticated tenant actor context.");
    error.code = "ORDER_DELETE_AUDIT_CONTEXT_REQUIRED";
    throw error;
  }
  const result = await rpc("delete_order_with_audit", {
    p_order_id: String(id || "").trim(),
    p_tenant_id: context.tenantId,
    p_actor_user_id: actorUserId,
    p_actor_role: actorRole,
    p_request_metadata: audit.requestMetadata && typeof audit.requestMetadata === "object"
      ? audit.requestMetadata
      : {}
  });
  const row = Array.isArray(result) ? result[0] : result;
  if (!row?.ok) {
    const error = new Error("Order was not found in the authenticated tenant.");
    error.code = "ORDER_DELETE_NOT_FOUND";
    throw error;
  }
  return row;
}
...
async function deleteCustomer(id, audit = {}) {
  const context = requireTenantContext("customers", "delete");
  const actorUserId = String(audit.actorUserId || audit.actor_user_id || context?.userId || "").trim();
  const actorRole = String(audit.actorRole || audit.actor_role || context?.tenantRole || "").trim();
  if (!context?.tenantId || !actorUserId || actorUserId !== String(context.userId || "").trim() || !actorRole) {
  ...
  const result = await rpc("delete_customer_with_audit", {
    p_customer_id: String(id || "").trim(),
    p_tenant_id: context.tenantId,
    p_actor_user_id: actorUserId,
    p_actor_role: actorRole,
    p_request_metadata: audit.requestMetadata && typeof audit.requestMetadata === "object"
  ...
  const row = Array.isArray(result) ? result[0] : result;
  if (!row?.ok) {
    const error = new Error("Customer was not found in the authenticated tenant.");
    error.code = "CUSTOMER_DELETE_NOT_FOUND";
    throw error;
  }
  return row;
}
...
async function cleanupImportJob(jobId, audit = {}) {
  const context = requireTenantContext("settings", "delete");
  ...
  const result = await rpc("cleanup_import_job_with_audit", {
    p_job_id: String(jobId || "").trim(),
    p_tenant_id: context.tenantId,
    p_actor_user_id: actorUserId,
    p_actor_role: actorRole,
    p_request_metadata: audit.requestMetadata && typeof audit.requestMetadata === "object"
  ...
  const row = Array.isArray(result) ? result[0] : result;
  if (!row?.ok) {
    const error = new Error("Import cleanup job was not found in the authenticated tenant.");
    error.code = "IMPORT_CLEANUP_NOT_FOUND";
    throw error;
  }
  return {
    job,
    deletedOrders: Number(row.deleted_orders || 0),
    deletedCustomers: Number(row.deleted_customers || 0),
    deletedImportRecords: Number(row.deleted_import_records || 0),
    supported: true
  };
}
...
async function importOrdersBatch(rows, options = {}) {
  const validRows = [];
  const failed = [];
...
function orderMetadataFromRawText(rawText) {
  try {
    ...
    return {
      ...
      importJobId: parsed.__importJobId || (nested ? orderMetadataFromRawText(nested).importJobId : "") || "",
...
function rawTextWithOrderMetadata(order) {
  ...
  return JSON.stringify({
    ...
    __importJobId: order.importJobId || "",
...
async function readDb() {
  ...
  const originalCustomers = (mapped.customers || []).map(customer => ({
    ...customer,
    tags: Array.isArray(customer.tags) ? [...customer.tags] : customer.tags
  }));
...
async function persistOrderMutation(change = {}, settings = null) {
  ...
  await Promise.all([
    ...
    change.deletedOrderId
      ? timedStep("orderDeleteMs", () => {
        if (!change.deletionAudit) {
          const error = new Error("Order deletion audit context is required.");
          error.code = "ORDER_DELETE_AUDIT_REQUIRED";
          throw error;
        }
        return persistOrderDeletion(change.deletedOrderId, change.deletionAudit);
      })
...
async function persistHistoricalLineRecovery(input = {}) {
  const context = requireTenantContext("orders", "write");
  const tenantId = String(input.tenantId || input.tenant_id || "").trim();
  if (!tenantId || tenantId !== context.tenantId) {
    const error = new Error("Historical recovery tenant context mismatch.");
    error.code = "RECOVERY_TENANT_CONTEXT_MISMATCH";
    throw error;
  }
  const order = input.order || {};
  const customer = input.customer || {};
  const result = await rpc("recover_historical_line_order", {
    p_tenant_id: tenantId,
    p_line_event_id: String(input.lineEventId || ""),
    p_line_message_id: String(input.lineMessageId || ""),
    p_original_order_id: String(input.originalOrderId || ""),
    p_customer: customerRowPayload(customer),
    p_order: orderRowPayload(order),
    p_products_before: input.productsBefore ?? null,
    p_products_after: input.productsAfter ?? null,
    p_products_updated_at: input.productsUpdatedAt || null,
    p_customer_updated_at: input.customerUpdatedAt || null,
    p_source_snapshot: input.sourceSnapshot || {}
  });
  const row = Array.isArray(result) ? result[0] : result;
  const resolved = typeof row === "string" ? JSON.parse(row) : row;
  if (!resolved?.ok) {
    const error = new Error("Historical LINE recovery was not confirmed.");
    error.code = "LINE_RECOVERY_NOT_CONFIRMED";
    throw error;
  }
  return resolved;
}
