...
function tenantOwnedRow(table, row = {}) {
  ...
  return { ...stripTenantFields(row), tenant_id: context.tenantId };
}

function tenantScopedQuery(table, query = "", operation = "read") {
  const context = requireTenantContext(table, operation);
  if (!context) return query;
  const tenantFilter = `tenant_id=eq.${encodeURIComponent(context.tenantId)}`;
  if (!query) return tenantFilter;
  return query.includes("tenant_id=") ? query : `${query}&${tenantFilter}`;
}
...
async function request(table, options = {}, query = "") {
  ...
  if (TENANT_OWNED_TABLES.has(table)) {
    if (method === "POST") {
      ...
      query = tenantScopedQuery(table, query.replace(/^\?/, ""), "update");
      query = query ? `?${query}` : "";
    } else if (method === "DELETE") {
      query = tenantScopedQuery(table, query.replace(/^\?/, ""), "delete");
      query = query ? `?${query}` : "";
    }
  }
...
async function selectUsersForTenantContext() {
  ...
  return selectWhereUnscoped("users", `id=in.${encodeURIComponent(inFilter(userIds))}`);
}
...
async function resolveTenantForLineWebhook(body = {}) {
  ...
  const configuredChannelSecret = String(process.env.LINE_CHANNEL_SECRET || "").trim();
  const events = Array.isArray(body.events) ? body.events : [];
  const groupIds = [...new Set(events.map(event => String(event?.source?.groupId || "").trim()).filter(Boolean))];
  if (
    configuredTenantId
    && configuredChannelSecret
    && (
      !configuredGroupId
      || (groupIds.length === 1 && groupIds[0] === configuredGroupId)
    )
  ) {
    const tenants = await selectWhereUnscoped("tenants", `id=eq.${encodeURIComponent(configuredTenantId)}&status=eq.active&limit=1`).catch(() => []);
    if (tenants?.[0]) {
      return {
  ...
  const settingsRows = await selectWhereUnscoped(
    "settings",
    `key=eq.${encodeURIComponent("lineGroupId")}&value=eq.${encodeURIComponent(JSON.stringify(groupId))}`
  ).catch(() => []);
  const tenantIds = [...new Set((settingsRows || []).map(row => row.tenant_id).filter(Boolean))];
...
async function deleteUser(id) {
  ...
  if (!userId) return;
  const context = requireTenantContext("tenant_memberships", "delete");
...
async function writeDb(db) {
  ...
  if (tenantUsers.length) {
    const existingMemberships = await selectWhereUnscoped(
      "tenant_memberships",
      `tenant_id=eq.${encodeURIComponent(context.tenantId)}&user_id=in.${encodeURIComponent(inFilter(tenantUsers.map(user => user.id)))}`
    ).catch(() => []);
