...
async function settingsForImport() {
  const rows = await selectWhere("settings", `key=in.${encodeURIComponent("(products,productCosts,additionalCosts)")}`);
  const settings = {};
  for (const row of rows || []) settings[row.key] = row.value;
  settings.products = (Array.isArray(settings.products) ? settings.products : [])
    .filter(product => product && product.archived !== true && String(product.name || "").trim());
  settings.productCosts = Array.isArray(settings.productCosts) ? settings.productCosts : [];
  settings.additionalCosts = Array.isArray(settings.additionalCosts) ? settings.additionalCosts : [];
  return settings;
}
...
async function importOrdersBatch(rows) {
  ...
  const importSettings = await settingsForImport();
  const activeProducts = importSettings.products;
  ...
  for (const row of validRows) {
    ...
    Object.assign(order, importOrderProfitSnapshot(order, importSettings));
    const dbOrder = orderRowPayload({
      ...order,
      rawText: row.rawText || "",
    ...
    orders.push({ __rowNumber: row.rowNumber, __sourceRow: row, ...dbOrder });
...
async function readDb() {
  ...
  const mapped = fromSupabaseShape({ users, customers, orders, line_messages, follow_up_rules, settings, tags, customer_tags, contact_logs });
...
async function readOrderSaveContext() {
  const startedAt = Date.now();
  const tableTimings = {};
  async function timed(name, task) {
    const tableStartedAt = Date.now();
    const rows = await task();
    tableTimings[name] = Date.now() - tableStartedAt;
    return rows;
  }
  const settingsKeys = [
    "products",
  ...
  const [
    ...
    timed("customers", () => select("customers")),
    timed("orders", () => select("orders")),
    timed("follow_up_rules", () => select("follow_up_rules")),
    timed("settings", () => selectWhere("settings", `key=in.(${settingsKeys.map(encodeURIComponent).join(",")})`)),
    timed("tags", () => select("tags")),
    timed("customer_tags", () => select("customer_tags")),
    timed("contact_logs", () => select("contact_logs"))
  ]);
  const mapStartedAt = Date.now();
  const mapped = fromSupabaseShape({
    users: [],
    customers,
    orders,
    line_messages: [],
  ...
  const syncStartedAt = Date.now();
  const synced = synchronizeCustomers(mapped);
  readOrderSaveContext.lastTimings = {
    totalMs: Date.now() - startedAt,
    tables: tableTimings,
    mapMs,
    syncMs: Date.now() - syncStartedAt
...
async function readUserById(id) {
  ...
  if (!value) return null;
  const [users, avatarRows, themeRows] = await Promise.all([
