...
async function createContactLogFast(input = {}) {
  ...
  const [customers, orderRows, duplicateRows] = await Promise.all([
    ...
    canUseExactCycle
      ...
      : (manualOpportunityResult || requestedOrderId)
        ...
        : Promise.resolve([]),
    canUseExactCycle
      ? selectWhere("contact_logs", `customer_id=eq.${encodeURIComponent(customerId)}&result=eq.${encodeURIComponent(input.result || "")}&note=like.${encodeURIComponent(`*${exactMarker}*`)}`)
  ...
  if (manualOpportunityResult) {
    const existing = canUseExactCycle
      ? (duplicateRows || []).map(mapContactLogRow)[0]
