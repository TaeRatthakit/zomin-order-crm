...
function synchronizeCustomers(db, options = {}) {
  ...
  for (const order of orders) {
    ...
    const existingCustomer = existingById.get(requestedId) || (order.phone ? existingByPhone.get(order.phone) : null);
    const requestedCustomerExists = requestedId && existingById.has(requestedId);
    const groupId = requestedCustomerExists && !usedIds.has(requestedId)
      ? requestedId
    ...
    if (!grouped.has(groupId)) {
      grouped.set(groupId, {
        id: groupId,
        existing: existingById.get(groupId) || (order.phone ? existingByPhone.get(order.phone) : null) || {},
        orders: []
      });
      usedIds.add(groupId);
    }
