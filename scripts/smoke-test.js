...
async function main() {
  ...
  if (
    !upsaleReplyText.includes("✅ อัปเดตออเดอร์อัปเซลเรียบร้อยแล้ว")
    || !upsaleReplyText.includes("เลขออเดอร์:")
    || !upsaleReplyText.includes("📦 จำนวน")
    || !upsaleReplyText.includes("6 → 13 (+7)")
    || !upsaleReplyText.includes("💰 ยอดซื้อ")
    || !upsaleReplyText.includes("1,000 → 1,800 บาท (+800)")
    || !upsaleReplyText.includes("📈 เพิ่มยอดขาย")
    || !upsaleReplyText.includes("• +800 บาท")
    || !upsaleReplyText.includes("🎁 ของแถม")
    || !upsaleReplyText.includes("None → Mesh Bag")
    || !upsaleReplyText.includes("📝 หมายเหตุ")
